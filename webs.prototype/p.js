(function () {
    if (typeof (stlib.pixels) == "undefined") {
        stlib.pixels = {
            ...
            getPxcelParams: function (d, c) {
                var b = stlib.pixels.getCurrentURL();
                var a = stlib.pixels.trimURL(b);
                return " var pxcelData = { v0: '" + encodeURIComponent(d) + "', v1: '" + encodeURIComponent(stlib.pixels.getReferrerDomain()) + "', v3: '" + encodeURIComponent(a) + "', v4: '" + encodeURIComponent(b) + "' };"
            },
            getRnd: function () {
                return (new Date()).getTime()
            },
            getPxcelTag: function (c) {
                var b = stlib.pixels.getRnd();
                var a = window.top.location === window.location ? window.location.toString() : document.referrer;
                var d = a.split("/")[2];
                return "var pxscrpt = document.createElement('script'); pxscrpt.id = 'pxscrpt'; pxscrpt.async = true; pxscrpt.defer = true; pxscrpt.src = '//t.sharethis.com/1/d/t.dhj?rnd=" + b + "&cid=c010&dmn=" + d + "';document.body.appendChild(pxscrpt);"
            },
            ...
            getIframeContents: function (b) {
                var a = stlib.pixels.getRnd();
                var c = stlib.pixels.getPxcelParams(b, a);
                header = "<!DOCTYPE html><html><head><title>ShareThis Segmenter</title></head>";
                return header + '<body onload="' + stlib.pixels.getPxcelTag(b) + '"><script>' + c + "<\/script></body>"
            },
            createSegmentFrame: function () {
                if (stlib.pixels.segmentframe || document.getElementById("stSegmentFrame")) {
                    return
                }
                if (stlib.timeonpage) {
                    stlib.data.pageInfo.pixelFired = true;
                    stlib.timeonpage.pixeltimetofire = new Date().getTime()
                }
                try {
                ...
                if (b) {
                    var a = stlib.pixels.getIframeContents(b);
                    d.appendChild(stlib.pixels.segmentframe);
                    stlib.pixels.segmentframe.contentWindow.document.open().write(a);
                    stlib.pixels.segmentframe.contentWindow.document.close()
                }
            },
            pixelOptimizerCallback: function (a) {
                if (typeof (a) !== "undefined") {
                    stlib.data.set("stid", a.stid, "pageInfo");
                    if (a.status == "success") {
                        stlib.pixels.firePixel = true
                    } else {
                        stlib.pixels.firePixel = false
                    }
                }
            }
        };
